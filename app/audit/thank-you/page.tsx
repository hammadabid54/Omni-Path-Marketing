...
const STATUS_ICON = {
  pass: <CheckCircle2 className="h-4 w-4 text-blue-600" />,
      ...
      <Section>
        <div className="container-page text-center max-w-xl mx-auto py-20">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">
            No audit found. <em className="font-serif not-italic text-blue-600">Let&apos;s run one.</em>
          </h1>
          <p className="mt-4 text-neutral-900/70">Drop in your URL and we&apos;ll scan 20 SEO issues in 60 seconds.</p>
        ...
        <div className="container-page max-w-3xl">
          <ScrollReveal>
            ...
            <h1 className="text-4xl md:text-5xl font-bold leading-tight tracking-tight">
              Your report. <em className="font-serif not-italic text-blue-600">In 60 seconds.</em>
            </h1>
            <p className="mt-3 text-neutral-900/65">
              We scanned <span className="text-neutral-900/85">{result.url}</span> and ranked 20 SEO signals. Here&apos;s the headline.
            </p>
          </ScrollReveal>
          ...
          <ScrollReveal delay={0.1}>
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              <div className="bento bento-lg">
                <div className="text-xs uppercase tracking-widest text-neutral-900/55">Overall score</div>
                <motion.div
                ...
                >
                  {result.score}<span className="text-2xl text-neutral-900/40">/100</span>
                </motion.div>
              ...
              <div className="bento bento-lg">
                <div className="text-xs uppercase tracking-widest text-neutral-900/55">What we checked</div>
                <div className="mt-3 text-5xl font-bold text-neutral-900">{result.checks.length}</div>
                <div className="mt-1 text-sm text-neutral-900/60">SEO signals scanned</div>
                <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
                  <div>
                    <div className="text-2xl font-bold text-blue-600">
                      {result.checks.filter((c) => c.status === "pass").length}
                    </div>
                    <div className="text-neutral-900/55">Pass</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-amber-400">
                      {result.checks.filter((c) => c.status === "warn").length}
                    </div>
                    <div className="text-neutral-900/55">Warn</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-rose-400">
                      {result.checks.filter((c) => c.status === "fail").length}
                    </div>
                    <div className="text-neutral-900/55">Fail</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <ScrollReveal delay={0.15}>
            <div className="mt-10">
              ...
              <div className="mt-4 rounded-2xl border border-neutral-200/8 divide-y divide-white/8">
                {result.checks.map((c) => (
                  <div key={c.id} className="flex items-start gap-3 p-4">
                    <div className="mt-0.5 shrink-0">{STATUS_ICON[c.status]}</div>
                    <div className="flex-1">
                      <div className="font-medium text-neutral-900/90">{c.label}</div>
                      <div className="text-sm text-neutral-900/60">{c.detail}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <ScrollReveal delay={0.2}>
            <div className="mt-10 flex flex-wrap items-center gap-3">
              ...
              <Link href="/audit" className="inline-flex items-center gap-2 text-sm text-neutral-900/65 hover:text-blue-600 ml-auto">
      ...
      <Section>
        <div className="container-page max-w-3xl">
          <ScrollReveal>
            ...
            <ol className="mt-6 space-y-4 text-neutral-900/80">
              <li className="flex gap-4">
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-600/15 text-blue-600 text-sm font-semibold">1</span>
                <span><strong className="text-neutral-900">Email.</strong> The full PDF is on its way to your inbox now (check spam if you don&apos;t see it).</span>
              </li>
              <li className="flex gap-4">
              ...
              <li className="flex gap-4">
                ...
                <span><strong className="text-neutral-900">Plan.</strong> Get a 90-day plan with pricing. No obligation.</span>
              </li>
            </ol>
          </ScrollReveal>
        </div>
