    ...
    <main className="min-h-screen bg-white">
      <header className="border-b border-neutral-200/8 bg-white">
        <div className="container-page py-5 flex items-center justify-between gap-4">
          <Link href="/" className="text-neutral-900 font-semibold tracking-tight" aria-label="Omni Path — home">
            Omni Path
          </Link>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-600/10 border border-blue-600/30 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-blue-600">
      ...
      <section className="container-page py-10 md:py-16 max-w-3xl">
        <div className="mb-8">
          ...
          <h1 className="mt-2 text-3xl md:text-4xl font-bold leading-tight tracking-tight">
            Welcome. <em className="font-serif not-italic text-blue-600">Let&apos;s get you set up.</em>
          </h1>
          <p className="mt-3 text-neutral-900/65 max-w-2xl">
            ...
            <span className="text-neutral-900/85 font-medium">{new Date(expiresAt).toLocaleDateString()}</span>.
          </p>
          <p className="mt-2 text-[11px] text-neutral-900/40 font-mono">ref: {token}</p>
        </div>

        <IntakeForm token={token} />

        <div className="mt-10 grid gap-3 sm:grid-cols-2 text-sm text-neutral-900/55">
          <a href={`mailto:${BRAND_EMAIL}`} className="flex items-center gap-2 hover:text-blue-600">
            <Mail className="h-4 w-4" /> {BRAND_EMAIL}
          </a>
          <a href={BRAND_PHONE_TEL} className="flex items-center gap-2 hover:text-blue-600">
...
function ExpiredView({ token }: { token: string }) {
  return (
    <main className="min-h-screen bg-white flex items-center justify-center p-6">
      <div className="max-w-md w-full rounded-2xl border border-neutral-200/10 bg-[#11111A] p-8 text-center">
        <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-rose-400/10 text-rose-400">
          <Clock className="h-6 w-6" />
        </div>
        <h1 className="mt-4 text-2xl font-semibold text-neutral-900">This link has expired.</h1>
        ...
        <p className="mt-2 text-xs text-neutral-900/40 font-mono">ref: {token}</p>
        <a
...
function AlreadySubmittedView() {
  return (
    <main className="min-h-screen bg-white flex items-center justify-center p-6">
      <div className="max-w-md w-full rounded-2xl border border-blue-600/30 bg-blue-600/5 p-8 text-center">
        <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600/10 text-blue-600">
          <CheckCircle2 className="h-6 w-6" />
        </div>
        <h1 className="mt-4 text-2xl font-semibold text-neutral-900">Already submitted.</h1>
        <p className="mt-2 text-neutral-900/65 text-sm">
