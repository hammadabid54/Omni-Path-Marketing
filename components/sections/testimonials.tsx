          ...
          {items.map((t, i) => (
            <ScrollReveal key={i} delay={i * 0.05}>
              <figure className="bento">
                ...
                <blockquote className="mt-4 text-lg md:text-xl leading-relaxed text-neutral-900/85">
                  {t.quote}
                </blockquote>
                <figcaption className="mt-4 text-sm text-neutral-900/55">
                  <span className="text-neutral-900/85 font-medium">{t.attribution}</span>
                  {t.role && <> · {t.role}</>}
                  {t.highlight && (
